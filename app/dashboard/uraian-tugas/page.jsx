          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
            {loading ? (
              ...
              <div className="text-center py-20 opacity-20">
                <HiOutlineClipboardList size={48} className="mx-auto" />
                <p className="text-[10px] font-black uppercase mt-4">Belum ada rincian tugas</p>
              </div>
            )}
          </div>
        </div>
    ...
    <div className='flex flex-col gap-5'>
      <div className="space-y-4">
        <label className="block">
          <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2 block">Beban Kerja (1 Tahun)</span>
          <input 
            type="number" 
            value={values.jumlahBebanKerja}
            onChange={e => setValues({...values, jumlahBebanKerja: e.target.value})}
            placeholder="0"
        ...
        <div className="grid grid-cols-2 gap-4">
            <label className="block">
                <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2 block">Penyelesaian (JAM)</span>
                <input 
                    type="number" 
                    value={values.waktuPenyelesaian}
                    onChange={e => setValues({...values, waktuPenyelesaian: e.target.value})}
                    placeholder="0"
                    className="w-full bg-black/40 border border-white/10 rounded-xl py-3 px-4 text-sm text-white focus:outline-none focus:border-[#6d28d9] transition-all" 
                />
            </label>
            ...
            <label className="block">
                <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2 block">Waktu Efektif</span>
                <input 
                    type="number" 
                    value={values.waktuEfektif}
                    onChange={e => setValues({...values, waktuEfektif: e.target.value})}
                    placeholder="0"
                    className="w-full bg-black/40 border border-white/10 rounded-xl py-3 px-4 text-sm text-white focus:outline-none focus:border-[#6d28d9] transition-all" 
                />
            </label>
      ...
      >
        {isSubmit ? <Loader2 className="animate-spin" size={14} /> : <Save size={14} />}
