...
export default function LoginModal({ isOpen, onClose }) {
    ...
    return (
        <dialog className={`modal fixed inset-0 w-full h-full z-[999] backdrop-blur-md flex items-center justify-center ${isOpen ? 'modal-open' : ''}`}>
            ...
            <div className="modal-backdrop fixed inset-0 w-full h-full bg-black/40 z-[-1]" onClick={onClose}>
                <button className="w-full h-full cursor-default outline-none">close</button>
            </div>
        </dialog>
    );
}