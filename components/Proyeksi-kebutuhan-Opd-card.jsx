...
import { HiOutlineUserAdd, HiOutlineOfficeBuilding, HiOutlineClock, HiOutlineCalendar } from 'react-icons/hi';
...
const ProyeksiPegawaiCard = () => {
  ...
  const processedData = useMemo(() => {
    const grouped = rawData.reduce((acc, curr) => {
      if (!acc[curr.jabatan]) {
        acc[curr.jabatan] = { jabatan: curr.jabatan, abk: curr.abk, pegawai: [] };
      }
    ...
    return Object.values(grouped).map(item => {
      const perTahun = listTahun.map(year => {
        const listKeluar = item.pegawai.filter(p => cekTahunKeluar(p) === year);
        return {
          tahun: year,
      ...
      const tahunKritis = perTahun.find(t => t.jumlahKeluar > 0)?.tahun || 9999;
      ...
      <div className="mb-12">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-white/5 pb-8">
          <div className="flex flex-col sm:flex-row sm:items-center gap-6">
            <div>
              <h2 className="text-xl font-black italic text-white uppercase tracking-tighter leading-none">
                Proyeksi Kebutuhan <span className="text-indigo-500">Kekosongan.</span>
              </h2>
            ...
            <div className="flex items-center gap-3 bg-white/5 border border-white/10 px-3 py-2 rounded-2xl group hover:border-indigo-500/50 transition-colors">
              ...
              <div className="flex flex-col">
                <label className="text-[7px] font-black text-slate-500 uppercase italic">Mulai Tahun</label>
                <select 
                  value={startYear}
          ...
          <div className="flex flex-wrap gap-2">
            {listTahun.map((year) => {
              const totalKeluarTahunIni = rawData.filter(p => cekTahunKeluar(p) === year).length;
              return (
                <div key={year} className="flex flex-col items-center">
                  <div className={`px-3 py-1.5 rounded-xl border transition-all duration-300 ${
                    totalKeluarTahunIni > 0 
                    ? 'bg-rose-500/10 border-rose-500/20 shadow-[0_0_10px_rgba(244,63,94,0.1)]' 
                    : 'bg-white/5 border-white/10'
                  }`}>
                    <p className="text-[8px] font-black text-slate-500 uppercase leading-none mb-1">{year}</p>
                    <div className="flex items-center gap-1.5">
                      <div className={`w-1.5 h-1.5 rounded-full ${totalKeluarTahunIni > 0 ? 'bg-rose-500 animate-pulse' : 'bg-slate-700'}`} />
                      <span className={`text-xs font-black ${totalKeluarTahunIni > 0 ? 'text-white' : 'text-slate-600'}`}>
                        {totalKeluarTahunIni} <span className="text-[9px] font-normal opacity-60">Org</span>
                      </span>
                    </div>
                  </div>
                  {totalKeluarTahunIni > 0 && (
                    <div className="mt-1 px-2 py-0.5 bg-amber-500 rounded-full shadow-lg shadow-amber-500/20">
                      <p className="text-[7px] font-black text-black italic">KEB {year + 1}</p>
                    </div>
                  )}
                </div>
              );
        ...
        {processedData.map((jab, idx) => (
          <div key={idx} className="relative">
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <h3 className="text-lg font-black text-white uppercase tracking-tight leading-none">{jab.jabatan}</h3>
...
>
  ...
  <div className="relative z-10">
    <div className="flex justify-between items-center mb-5">
      <span className={`text-xs font-black font-mono tracking-widest ${isKritis ? 'text-rose-400' : 'text-slate-500'}`}>
        {p.tahun}
      </span>
      {isKritis && (
    ...
    {isKritis ? (
      <div className="space-y-5">
        <div className="flex items-baseline gap-1">
          <span className="text-4xl font-black text-white">{p.jumlahKeluar}</span>
          <span className="text-[10px] font-bold text-rose-400 uppercase italic">Pegawai Keluar</span>
        </div>
        ...
        <div className="bg-amber-500 text-black p-4 rounded-2xl shadow-md transform hover:translate-y-[-2px] transition-transform duration-200 border-b-4 border-amber-700">
          <div className="flex items-center gap-3">
            <HiOutlineUserAdd size={18} className="opacity-80" />
            <div className="leading-none">
              <p className="text-[8px] font-black uppercase opacity-70 mb-0.5">Kebutuhan Formasi</p>
              <p className="text-sm font-black italic uppercase">Tahun {p.tahun + 1}</p>
            </div>
          </div>
        </div>
        ...
        <div className="pt-3 space-y-2 border-t border-white/10">
          {p.pegawaiKeluar.map((orang, oIdx) => (
            <div key={oIdx} className="bg-black/30 p-2.5 rounded-xl border border-white/5 group hover:border-rose-500/30 transition-colors">
              <p className="text-[10px] font-bold text-slate-100 uppercase truncate">{orang.nama}</p>
              <p className="text-[8px] font-medium text-rose-400 uppercase italic mt-1 flex items-center gap-1 animate-pulse">
                ...
                {orang.statusKeluar ? orang.statusKeluar.alasan : 'PENSIUN 58 TH'}
              </p>
            </div>
          ))}
        </div>
